Attribute VB_Name = "modLang"
Option Explicit
' 語言模組：全部 UI 與記錄字串；g_Chinese=True 繁中否則英文，auto 跟系統
Public g_Chinese As Boolean
Private Declare Function GetUserDefaultUILanguage Lib "kernel32" () As Integer

Public Sub LangInit(ByVal mode As String) ' 語言初始化：auto 跟系統語系
    Dim m As String
    m = LCase$(Trim$(mode))
    Select Case m
        Case "zh", "zh-tw", "zh-cn", "zh-hk", "chs", "cht", "chinese"
            g_Chinese = True
        Case "en", "english"
            g_Chinese = False
        Case Else
            g_Chinese = ((GetUserDefaultUILanguage() And &H3FF) = &H4)
    End Select
End Sub

' ---- 托盤選單文字 ----
Public Function S_MenuAuto() As String ' 選單：自動切換勾選
    If g_Chinese Then S_MenuAuto = "自動切換" Else S_MenuAuto = "Auto switch"
End Function
Public Function S_MenuFireOff() As String ' 選單：手動跑斷電鏈
    If g_Chinese Then S_MenuFireOff = "立即執行斷電流程" Else S_MenuFireOff = "Run power-off now"
End Function
Public Function S_MenuFireOn() As String ' 選單：手動跑通電鏈
    If g_Chinese Then S_MenuFireOn = "立即執行通電流程" Else S_MenuFireOn = "Run power-on now"
End Function
Public Function S_MenuHdrOn() As String ' 選單：直接開 HDR
    If g_Chinese Then S_MenuHdrOn = "開啟 HDR" Else S_MenuHdrOn = "Turn HDR On"
End Function
Public Function S_MenuHdrOff() As String ' 選單：直接關 HDR
    If g_Chinese Then S_MenuHdrOff = "關閉 HDR" Else S_MenuHdrOff = "Turn HDR Off"
End Function
Public Function S_MenuClean() As String ' 選單：手動清虛擬卡
    If g_Chinese Then S_MenuClean = "清除虛擬顯示卡…" Else S_MenuClean = "Clean virtual GPUs…"
End Function
Public Function S_MenuSettings() As String ' 選單：開設定窗
    If g_Chinese Then S_MenuSettings = "設定…" Else S_MenuSettings = "Settings…"
End Function
Public Function S_MenuOpenLog() As String ' 選單：開記錄檔位置
    If g_Chinese Then S_MenuOpenLog = "開啟紀錄資料夾" Else S_MenuOpenLog = "Open log folder"
End Function
Public Function S_MenuExit() As String ' 選單：結束程式
    If g_Chinese Then S_MenuExit = "結束程式" Else S_MenuExit = "Exit"
End Function

' ---- 托盤懸停提示 ----
Public Function S_TipStarting() As String ' 懸停：啟動中
    If g_Chinese Then S_TipStarting = "AutoHdrMode 啟動中" Else S_TipStarting = "AutoHdrMode starting"
End Function
Public Function S_TipState(ByVal isOn As Boolean) As String ' 懸停：開關狀態
    If g_Chinese Then
        If isOn Then S_TipState = "AutoHdrMode：螢幕開著" Else S_TipState = "AutoHdrMode：螢幕關了"
    Else
        If isOn Then S_TipState = "AutoHdrMode: display ON" Else S_TipState = "AutoHdrMode: display OFF"
    End If
End Function

' ---- 記錄檔字串 ----
Public Function S_LogStart(ByVal poll As Long, ByVal stable As Long, ByVal autoOn As Boolean) As String ' 記錄：常駐啟動參數
    If g_Chinese Then
        S_LogStart = "程式啟動：每 " & poll & " 秒檢查一次，連續 " & stable & " 次相同才算數，自動切換＝" & IIf(autoOn, "開", "關")
    Else
        S_LogStart = "Start: probe every " & poll & "s, " & stable & " in a row to confirm, auto " & IIf(autoOn, "on", "off")
    End If
End Function
Public Function S_LogAutoToggle(ByVal autoOn As Boolean) As String ' 記錄：自動開關異動
    If g_Chinese Then S_LogAutoToggle = "自動切換改為：" & IIf(autoOn, "開", "關") Else S_LogAutoToggle = "Auto switch: " & IIf(autoOn, "on", "off")
End Function
Public Function S_LogInit(ByVal isOn As Boolean, ByVal d6 As String, ByVal nm As String) As String ' 記錄：初始狀態
    If g_Chinese Then
        S_LogInit = nm & "初始狀態：" & IIf(isOn, "開", "關") & "（" & d6 & "）"
    Else
        S_LogInit = nm & " initial: " & IIf(isOn, "ON", "OFF") & " (" & d6 & ")"
    End If
End Function
Public Function S_LogEvent(ByVal isOn As Boolean, ByVal d6 As String, ByVal nm As String) As String ' 記錄：翻轉事件
    If g_Chinese Then
        S_LogEvent = nm & "：" & IIf(isOn, "開", "關") & "（" & d6 & "）"
    Else
        S_LogEvent = nm & ": " & IIf(isOn, "ON", "OFF") & " (" & d6 & ")"
    End If
End Function
Public Function S_LogDirDisabled(ByVal isOn As Boolean) As String ' 記錄：方向被停用
    If g_Chinese Then
        S_LogDirDisabled = IIf(isOn, "通電", "斷電") & "自動動作沒開，這次跳過"
    Else
        S_LogDirDisabled = IIf(isOn, "Power-on", "Power-off") & " auto action off, skipped"
    End If
End Function
Public Function S_LogPlanHdr(ByVal isOn As Boolean, ByVal d As Long) As String ' 記錄：排程延遲秒數
    If g_Chinese Then
        S_LogPlanHdr = IIf(isOn, "通電", "斷電") & "流程開始，" & d & " 秒後動 HDR"
    Else
        S_LogPlanHdr = IIf(isOn, "Power-on", "Power-off") & ": HDR in " & d & "s"
    End If
End Function
Public Function S_LogSkip(ByVal isOn As Boolean) As String ' 記錄：排程中途取消
    If g_Chinese Then S_LogSkip = "狀態又變了，上次排的不做了" Else S_LogSkip = "State flipped again, dropping plan"
End Function
Public Function S_LogCancel(ByVal wasOn As Boolean) As String ' 記錄：排程被覆寫取消
    If g_Chinese Then
        S_LogCancel = IIf(wasOn, "通電", "斷電") & "流程取消：中途狀態又變，取消流程"
    Else
        S_LogCancel = IIf(wasOn, "Power-on", "Power-off") & " plan cancelled"
    End If
End Function
Public Function S_LogNativeTryOn() As String ' 記錄：嘗試開 HDR
    If g_Chinese Then S_LogNativeTryOn = "正在開啟 HDR…" Else S_LogNativeTryOn = "Turning HDR on..."
End Function
Public Function S_LogNativeTryOff() As String ' 記錄：嘗試關 HDR
    If g_Chinese Then S_LogNativeTryOff = "正在關閉 HDR…" Else S_LogNativeTryOff = "Turning HDR off..."
End Function
Public Function S_LogNativeResult(ByVal ok As Boolean, ByVal st As Long) As String ' 記錄：原生 HDR 結果
    Dim s As String
    Select Case st
        Case 0: s = IIf(g_Chinese, "不支援", "UNSUPPORTED")
        Case 1: s = IIf(g_Chinese, "關", "OFF")
        Case 2: s = IIf(g_Chinese, "開", "ON")
        Case 3: s = IIf(g_Chinese, "失敗", "FAILED")
        Case Else: s = IIf(g_Chinese, "未知", "UNKNOWN")
    End Select
    If g_Chinese Then
        S_LogNativeResult = "HDR 結果：" & IIf(ok, "成功", "失敗") & "，HDR現在是" & s
    Else
        S_LogNativeResult = "HDR: " & IIf(ok, "ok", "failed") & ", HDR now " & s
    End If
End Function
Public Function S_LogNativeSkip() As String ' 記錄：略過 HDR
    If g_Chinese Then S_LogNativeSkip = "HDR 設為略過，不動它" Else S_LogNativeSkip = "HDR set to skip"
End Function
Public Function S_LogVerifyWait(ByVal sec As Long) As String ' 記錄：驗證等待秒數
    If g_Chinese Then S_LogVerifyWait = "等 " & sec & " 秒再驗收" Else S_LogVerifyWait = "Verify in " & sec & "s"
End Function
Public Function S_LogVerifyOk() As String ' 記錄：驗收通過
    If g_Chinese Then S_LogVerifyOk = "驗收通過" Else S_LogVerifyOk = "Verify passed"
End Function
Public Function S_LogVerifyFail() As String ' 記錄：驗收失敗
    If g_Chinese Then S_LogVerifyFail = "驗收沒過：HDR 不是要的狀態" Else S_LogVerifyFail = "Verify failed: state mismatch"
End Function
Public Function S_LogCleanStart() As String ' 記錄：開始清卡
    If g_Chinese Then S_LogCleanStart = "開始清虛擬顯示卡…" Else S_LogCleanStart = "Cleaning virtual displays..."
End Function
Public Function S_LogCleanNone() As String ' 記錄：無卡可清
    If g_Chinese Then S_LogCleanNone = "沒有虛擬卡要清" Else S_LogCleanNone = "No virtual displays"
End Function
Public Function S_LogCleanRemove(ByVal name As String) As String ' 記錄：移除某卡
    If g_Chinese Then S_LogCleanRemove = "拔掉這台：" & name Else S_LogCleanRemove = "Removing: " & name
End Function
Public Function S_LogCleanRemoveRc(ByVal rc As Long) As String ' 記錄：移除結束碼
    If g_Chinese Then S_LogCleanRemoveRc = "拔卡結果，碼＝" & rc Else S_LogCleanRemoveRc = "Remove rc=" & rc
End Function
Public Function S_LogCleanOk() As String ' 記錄：清卡完成
    If g_Chinese Then S_LogCleanOk = "虛擬卡清完了" Else S_LogCleanOk = "All virtual displays cleared"
End Function
Public Function S_LogCleanTimeout() As String ' 記錄：清卡逾時
    If g_Chinese Then S_LogCleanTimeout = "清卡等太久，先往下走" Else S_LogCleanTimeout = "Clean timed out, moving on"
End Function
Public Function S_LogCleanErr(ByVal m As String) As String ' 記錄：清卡異常
    If g_Chinese Then S_LogCleanErr = "清卡出錯：" & m Else S_LogCleanErr = "Clean error: " & m
End Function
Public Function S_LogCleanElevate() As String ' 記錄：準備提權
    If g_Chinese Then S_LogCleanElevate = "需要管理員權限，正在提權…" Else S_LogCleanElevate = "Need admin rights, elevating..."
End Function
Public Function S_LogCleanElevateFail(ByVal rc As Long) As String ' 記錄：提權失敗
    If g_Chinese Then
        S_LogCleanElevateFail = "提權沒過（碼＝" & rc & "），請自己用管理員跑一次 --clean"
    Else
        S_LogCleanElevateFail = "Elevate failed (rc=" & rc & "); run --clean as admin manually"
    End If
End Function
Public Function S_LogCleanDone(ByVal rc As Long) As String ' 記錄：清卡結束碼
    If g_Chinese Then S_LogCleanDone = "清卡結束，結果碼＝" & rc Else S_LogCleanDone = "Clean finished, rc=" & rc
End Function
Public Function S_LogRetryEnable() As String ' 記錄：清後重試 HDR
    If g_Chinese Then S_LogRetryEnable = "清完重開一次 HDR" Else S_LogRetryEnable = "Retrying HDR after clean"
End Function
Public Function S_LogWaitAfterClean(ByVal sec As Long) As String ' 記錄：清後等待秒數
    If g_Chinese Then S_LogWaitAfterClean = "清完等 " & sec & " 秒再重開" Else S_LogWaitAfterClean = "Wait " & sec & "s after clean"
End Function
Public Function S_LogShellSkipEmpty() As String ' 記錄：Shell 空白跳過
    If g_Chinese Then S_LogShellSkipEmpty = "Shell 空白或不存在，不跑額外指令" Else S_LogShellSkipEmpty = "Shell empty or missing, skipped"
End Function
Public Function S_LogShellSkipMissing(ByVal p As String) As String ' 記錄：Shell 目標遺失
    If g_Chinese Then S_LogShellSkipMissing = "Shell 檔案不在了，跳過：" & p Else S_LogShellSkipMissing = "Shell target missing, skipped: " & p
End Function
Public Function S_LogShellRun(ByVal rc As Long) As String ' 記錄：Shell 結束碼
    If g_Chinese Then S_LogShellRun = "Shell 跑完，結束碼＝" & rc Else S_LogShellRun = "Shell finished, rc=" & rc
End Function
Public Function S_LogShellLaunch() As String ' 記錄：Shell 已啟動不等候
    If g_Chinese Then S_LogShellLaunch = "Shell 已啟動，不等它跑完" Else S_LogShellLaunch = "Shell launched, not waiting"
End Function
Public Function S_LogPhaseDone(ByVal isOn As Boolean, ByVal nm As String) As String ' 記錄：整鏈完成
    If g_Chinese Then S_LogPhaseDone = nm & " " & IIf(isOn, "通電流程結束", "斷電流程結束") Else S_LogPhaseDone = nm & " " & IIf(isOn, "power-on done", "power-off done")
End Function
Public Function S_LogDdcErr(ByVal m As String) As String ' 記錄：偵測錯誤（舊名保留）
    If g_Chinese Then S_LogDdcErr = "螢幕偵測出錯（DDC）：" & m Else S_LogDdcErr = "Display probe error (DDC): " & m
End Function
Public Function S_LogPollErr(ByVal m As String) As String ' 記錄：輪詢異常
    If g_Chinese Then S_LogPollErr = "偵測出錯：" & m Else S_LogPollErr = "Probe error: " & m
End Function
Public Function S_LogAutoErr(ByVal m As String) As String ' 記錄：自動鏈異常
    If g_Chinese Then S_LogAutoErr = "流程出錯：" & m Else S_LogAutoErr = "Flow error: " & m
End Function
Public Function S_LogOpenLogErr(ByVal m As String) As String ' 記錄：開記錄檔失敗
    If g_Chinese Then S_LogOpenLogErr = "打不開記錄資料夾：" & m Else S_LogOpenLogErr = "Cannot open log folder: " & m
End Function
Public Function S_LogProbe(ByVal onN As Long, ByVal sbN As Long, ByVal d6 As String) As String ' 記錄：探測結果行
    If g_Chinese Then S_LogProbe = "手動探測：亮著 " & onN & " 台，待命 " & sbN & " 台，" & d6 Else S_LogProbe = "Probe: " & onN & " lit, " & sbN & " standby, " & d6
End Function
Public Function S_LogAsIn() As String ' 記錄：自啟寫入
    If g_Chinese Then S_LogAsIn = "開機自動啟動：已寫入" Else S_LogAsIn = "Autostart: on"
End Function
Public Function S_LogAsRm() As String ' 記錄：自啟刪除
    If g_Chinese Then S_LogAsRm = "開機自動啟動：已刪除" Else S_LogAsRm = "Autostart: off"
End Function
Public Function S_LogAsErr(ByVal m As String) As String ' 記錄：自啟異常
    If g_Chinese Then S_LogAsErr = "開機自動啟動失敗：" & m Else S_LogAsErr = "Autostart failed: " & m
End Function
Public Function S_LogExit() As String ' 記錄：程式結束
    If g_Chinese Then S_LogExit = "程式結束" Else S_LogExit = "Exit"
End Function
Public Function S_LogManual(ByVal isOn As Boolean) As String ' 記錄：手動觸發
    If g_Chinese Then S_LogManual = "手動執行：" & IIf(isOn, "通電流程", "斷電流程") Else S_LogManual = "Manual: power-" & IIf(isOn, "on", "off")
End Function
Public Function S_LogNativeOffNoPath() As String ' 記錄：斷電無路徑提示
    If g_Chinese Then S_LogNativeOffNoPath = "斷電但一時抓不到螢幕，改用上次記的再關一次" Else S_LogNativeOffNoPath = "No display path; retried cached targets"
End Function
Public Function S_LogHdrManual(ByVal turningOn As Boolean) As String ' 記錄：手動 HDR
    If g_Chinese Then
        S_LogHdrManual = "手動" & IIf(turningOn, "開", "關") & " HDR"
    Else
        S_LogHdrManual = "Manual HDR " & IIf(turningOn, "on", "off")
    End If
End Function
Public Function S_LogSettingsSaved() As String ' 記錄：設定已存檔
    If g_Chinese Then S_LogSettingsSaved = "設定已儲存" Else S_LogSettingsSaved = "Settings saved"
End Function

' ---- 氣球與待命 ----
Public Function S_MenuBalloon() As String ' 選單：氣球提示勾選
    If g_Chinese Then S_MenuBalloon = "氣球提示" Else S_MenuBalloon = "Balloon tip"
End Function

Public Function S_LogBalloonToggle(ByVal isOn As Boolean) As String ' 記錄：氣球開關異動
    If g_Chinese Then
        If isOn Then S_LogBalloonToggle = "氣球提示：開" Else S_LogBalloonToggle = "氣球提示：關"
    Else
        If isOn Then S_LogBalloonToggle = "Balloon tip: on" Else S_LogBalloonToggle = "Balloon tip: off"
    End If
End Function

Public Function S_TipStandby() As String ' 懸停：待命中提示
    If g_Chinese Then S_TipStandby = "AutoHdrMode：螢幕待命中" Else S_TipStandby = "AutoHdrMode: display STANDBY"
End Function

Public Function S_LogHoldStandby(ByVal onN As Long, ByVal sbN As Long, ByVal d6 As String) As String ' 記錄：待命凍結
    If g_Chinese Then S_LogHoldStandby = "螢幕待命中，先不動（亮著 " & onN & " 台，待命 " & sbN & " 台，" & d6 & "）" Else S_LogHoldStandby = "Standby, holding (" & onN & " lit, " & sbN & " standby, " & d6 & ")"
End Function

Public Function S_LogWaitShell(ByVal secs As Long) As String ' 記錄：Shell 前等待秒數
    If g_Chinese Then S_LogWaitShell = "跑 Shell 前再等 " & secs & " 秒" Else S_LogWaitShell = "Wait " & secs & "s before shell"
End Function

