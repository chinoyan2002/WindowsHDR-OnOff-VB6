...
Public Function S_LogCancel(ByVal wasOn As Boolean) As String ' 記錄：排程被覆寫取消
    If g_Chinese Then
        S_LogCancel = IIf(wasOn, "通電", "斷電") & "流程取消：中途狀態又變，這次不做"
    Else
...
Public Function S_LogNativeResult(ByVal ok As Boolean, ByVal st As Long) As String ' 記錄：原生 HDR 結果
    ...
    End Select
    If g_Chinese Then
        S_LogNativeResult = "HDR 結果：" & IIf(ok, "成功", "失敗") & "，現在是" & s
    Else
...
Public Function S_LogShellSkipEmpty() As String ' 記錄：Shell 空白跳過
    If g_Chinese Then S_LogShellSkipEmpty = "沒填 Shell，不跑額外指令" Else S_LogShellSkipEmpty = "No Shell configured, skipped"
End Function
