...
### 6.2.3 螢幕名稱（事件／初始／收尾行）

`modDDC.DisplayName0()` 取 0 號 `HDR_DISPLAY_INFO.Name`；成功寫入 `m_CachedName`，失敗回快取，再沒有回「螢幕」。只在翻轉／初始／收尾呼叫，不進每輪 poll。`DdcD6Summary` 單台純量（`D6=1`／`D6=問不到`），多台列表。
...
7. `.vbp` 不可留 `Path32`（從別處複製 vbp 會帶來）；殘留會把 exe 輸出到別目錄，甚至蓋掉別人的 exe。若發生，删該行重編，並幫受害目錄重編還原。
8. `Private` 模組變數放程序區之前；放程序後會報「註解只能放在 End Sub 後面」。
9. 每次 build 驗新鮮度：清 `%TEMP%\VB*`＋删 exe＋重編＋跑 `--probe` 對版本行（vbp Revision-1）；對不上就停手查，不准憑感覺認定。
...
| 2026-09-20 | 誤報事件記入 §15：通電暖機 DDC 盲區造成開→關→開；DelayShell／ShellTimeout／取消記錄／白話文上線 |
